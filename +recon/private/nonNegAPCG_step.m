function [x,infostruc] = nonNegAPCG_step(A,b,varargin)
...
%             end
        ...
        a_step=10;%prs.Results.a_step;
        a_step = calculateStepsize(d.*I1,a_step,g,A*xi,A*(d.*I1));
        %% Update and project
...
%        imagesc(reshape(xi,[sqrt(numel(xi)) sqrt(numel(xi))])); colormap gray; drawnow;
 ...
 function armijoAlpha = calculateStepsize(pdir,alpha,locGrad,Ax,Ap)
        tau = 0.5;
        c = 0.1;
        ...
        slope = dot(pdir,locGrad);
        expectedLoss = slope*c;
