...
%% This testing file should cover a ton of cases using the substitution parser.



% Create a metadata object which has some properties we'd be interested in.
metaObj.CellArrayOfData = {randn(2,2),ones(2),magic(2)};
metaObj.CellArrayOfParams = {@mtimes,magic(3),rand(3) + 2};
metaObj.CellArrayOfNameValues = {'N1',1.0,'N2',{'V2'},'N3','V3','N4',true};

Obj.testFunc = @mpower;
Obj.testString = "Hi";
Obj.deepReplaceString = "eval:(2^eval:(sqrt(2)))";
Obj.Meta = metaObj;

frame.Data = randn(5,4);
frame.Meta.Index = 5;

testBundle.repString1 = "{obj:Meta.CellArrayOfData}";
testBundle.repString2 = "eval:(cat(3,{meta:CellArrayOfData}))";
testBundle.subStruct.repString1 = "eval:(eval:(2))";


testBundle.framePhase = "frame:Meta.Index";



cellBundle = {testBundle.repString1,testBundle.repString2,testBundle.subStruct.repString1};

labeledCellBundle = {"testBundle.repString1","{obj:Meta.CellArrayOfData}",... % Tests expansion of the cell array. Since this is not going inside of an eval  string, it should be rewrapped as a cell array. 
                     "testBundle.repString2", "eval:(cat(3,{meta:CellArrayOfData}))",...
                     "testBundle.subStruct.repString1", "eval:(eval:(2))"};

replaceVals(Obj,testBundle);
replaceVals(Obj,cellBundle);
replaceVals(Obj,labeledCellBundle);
...
function [filledString] = replaceString(obj,repString,doStringConversion)

    filledString = repString;
    % Check for eval calls.
    [a,b,c,d,e,f] = regexp(repString,'.{0}(?:eval:)\({1}(.*)\){1}.{0}','match','tokens');
    
    if ~isempty(b) % There's at least one eval call we need to handle. 
        evalContents = b{1};
